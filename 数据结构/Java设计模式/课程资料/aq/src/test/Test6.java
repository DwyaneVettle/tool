package test;

import util.RSACodeUtil;

public class Test6 {
	public static void main(String[] args) throws Exception {
		String str = "大家辛苦。。。";
//		String publicKeyStr = RSACodeUtil.getPublicKeyStr();
//		String privateKeyStr = RSACodeUtil.getPrivateKeyStr();
		
		String publicKeyStr = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCnV7zQ238e0FWp2WqulJTlAQRSLf+af8dCxVBUhDGhRVh1wjZ9d1ms3DWq4orfWRqeRrlzZSfx2BrIK5H/DiCwTZ8n9LTiXN/GKiYDHnC4WgtB7I3zczXJ/X+vQ7irgfgRYS1qrN2YE/PSMUPCGV18yA82Ht8gQiP79PI+CVTY4wIDAQAB";
		String privateKeyStr = "";
		System.out.println("公钥：" + publicKeyStr);
		System.out.println("私钥：" + privateKeyStr);
		
		//1.私钥加密  公钥解密
//		String encoder2PrivateKey = RSACodeUtil.encoder2PrivateKey(str, privateKeyStr);
//		System.out.println("私钥密文：" + encoder2PrivateKey);
//		
//		String decoder2PublicKey = RSACodeUtil.decoder2PublicKey(encoder2PrivateKey, publicKeyStr);
//		System.out.println("公钥解密：" + decoder2PublicKey);
		
		//2.公钥加密  私钥解密
		String encoder2PublicKey = RSACodeUtil.encoder2PublicKey(str, publicKeyStr);
		System.out.println("公钥加密：" + encoder2PublicKey);
		
		String decoder2PrivateKey = RSACodeUtil.decoder2PrivateKey(encoder2PublicKey, privateKeyStr);
		System.out.println("私钥解密：" + decoder2PrivateKey);
		
	}
}
